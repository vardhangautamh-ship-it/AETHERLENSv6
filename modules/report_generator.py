...
def _build_next_steps_section(agent_results: dict = None, person: dict = None) -> dict:
    ...
    if ns_data and (ns_data.get("next_steps") or ns_data.get("steps")):
        ...
        for i, step in enumerate(steps[:8], 1):
            if isinstance(step, dict):
                ...
                from modules.statute_era import enforce_new_era as _era
                lines.append(_era(
                    f"STEP {n}: {action}"
    ...
    if not person.get("phones_found"):
        lines.append(
            ...
            f"Legal basis: BNSS Section 94 / IT Act Sec 69. Authorization: Court order. "
    ...
    if not person.get("location_stated"):
        lines.append(
            ...
            f"Legal basis: TRAI regulations / BNSS Section 94. Authorization: ACP approval. "
    ...
    lines.append(
        ...
        f"Legal basis: BNSS Section 35. Authorization: Inspector level and above. "
...
def _build_timeline_intelligence_section(
    ...
    if timeline_events:
        for ev in timeline_events[:30]:
            ...
            date_str  = ev.get("date") or ev.get("display_date") or ev.get("normalized", "")
...
def _build_sections_local(
    ...
    locations  = person.get("location_stated", [])
    events     = [
        f"{e.get('display_date', e['normalized'])}: "
...
def _case_type_family(label) -> str:
    """Normalise any case-type label to the §09B type token it belongs to:
    'Financial Crime' → 'financial', 'Cyber Crime' → 'cyber', 'Immigration
    Offence' → 'immigration'; anything else (General OSINT, Drug/Narcotics,
    empty) → 'general'. Used to compare §18's label with §09B's type."""
    s = str(label or "").lower()
    for fam in ("immigration", "financial", "cyber"):
        if fam in s:
            return fam
    return "general"
...
def _generate_report_inner(
    ...
    # Single source of truth: the orchestrator's plan was generated BEFORE
    # §09B ran, so it derived its own case type from anomaly tokens. If that
    # label contradicts the authoritative §09B type, discard it and
    # regenerate below with the §09B hint — one report, one case type.
    _s09b_ct = str(
    ...
    if (isinstance(tactical_plan_result, dict) and tactical_plan_result.get("actions")
            and _s09b_ct in ("financial", "cyber", "immigration")
            and _case_type_family(tactical_plan_result.get("case_type")) != _s09b_ct):
        print(f"[TACTICAL_PLAN] case type {tactical_plan_result.get('case_type')!r} "
              f"contradicts §09B {_s09b_ct!r} — regenerating with authoritative type")
    ...
    # BUILD-TIME ASSERT — §09B case type and §18 case type must agree. A
    # mismatch after the reconciliation above is a code bug: surface it as an
    # error rather than shipping a self-contradicting report.
    _tp_final = sections.get("tactical_plan") or {}
    if (_tp_final.get("actions")
            and _s09b_ct in ("financial", "cyber", "immigration")
            and _case_type_family(_tp_final.get("case_type")) != _s09b_ct):
        raise RuntimeError(
            f"CASE-TYPE CONSISTENCY VIOLATION: §09B detected {_s09b_ct!r} but "
            f"§18 carries {_tp_final.get('case_type')!r}. Refusing to ship a "
            f"self-contradicting report.")
